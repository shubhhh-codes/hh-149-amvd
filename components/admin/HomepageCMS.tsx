  ...
  const handleSavePerformer = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (selectedPerformer) {
        // Update existing performer profile
        ...
        const data = await res.json();
        if (!res.ok) throw new Error(data.message || 'Failed to add performer');
          ...
          {cmsTab === 'performers' && (
            <div className="space-y-4">
              <div className="flex justify-end">
                <button 
                  onClick={() => { setSelectedPerformer(null); setShowPerformerModal(true); }}
      ...
      {showPerformerModal && (
        <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-[100] backdrop-blur-sm p-4 overflow-y-auto">
          <div className="bg-surface-container-high brutal-border rounded-lg p-6 w-full max-w-xl my-8">
            <h2 className="text-xl font-headline-md mb-6">{selectedPerformer ? `Edit Performer: ${selectedPerformer.username}` : 'Add New Performer'}</h2>
            <form onSubmit={handleSavePerformer} className="space-y-4">
              {!selectedPerformer && (
                <>
                  <div>
                    ...
                    <input name="email" type="email" required className="w-full bg-[#080808] border border-outline-variant p-2 rounded text-white" />
                  </div>
                  <div>
                    <label className="block text-sm font-label-caps text-on-surface-variant mb-1">Phone Number (Optional)</label>
                    <input name="phone" type="tel" className="w-full bg-[#080808] border border-outline-variant p-2 rounded text-white" />
                  </div>
                </>
              )}
              <div>
                <label className="block text-sm font-label-caps text-on-surface-variant mb-1">Display Name *</label>
                <input name="name" defaultValue={selectedPerformer?.username || ''} required className="w-full bg-[#080808] border border-outline-variant p-2 rounded text-white" />
              </div>
              <div>
                <label className="block text-sm font-label-caps text-on-surface-variant mb-1">Speciality *</label>
                <input name="speciality" defaultValue={selectedPerformer?.comedianProfile?.speciality || ''} required className="w-full bg-[#080808] border border-outline-variant p-2 rounded text-white" />
              </div>
              <div>
              ...
              <div>
                <label className="block text-sm font-label-caps text-on-surface-variant mb-1">{selectedPerformer ? 'Upload New Photo (JPG/PNG)' : 'Upload Photo (JPG/PNG)'}</label>
                ...
                <p className="text-xs text-on-surface-variant mt-1">{selectedPerformer ? 'Leave empty to keep existing photo.' : 'Optional.'} Images are auto-converted to WebP.</p>
              </div>
              ...
              <div className="flex justify-end gap-3 mt-6">
                ...
                <button type="submit" disabled={uploadingImage} className="px-4 py-2 bg-primary text-on-primary font-headline-md rounded hover:brightness-110 disabled:opacity-50">
                  {uploadingImage ? 'Saving...' : selectedPerformer ? 'Save Changes' : 'Add Performer'}
                </button>
              </div>
            </form>
          </div>
